...
let changedElem = [temp, minTemp, maxTemp, feelingTemp, windSpeed]
...
const convertUnit = () => {
    ...
    changedElem.forEach((ele) => {
      let textAnimation = (0,animejs_lib_anime_es__WEBPACK_IMPORTED_MODULE_0__["default"])({
        ...
        begin: () => {
          changedElem.forEach((ele) => {
            ele.style.filter = 'brightness(1.2)';
          })
        },
      });
    });
...
async function weatherRequest(city) {
    try {
      let response = await fetch(
        `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=7a6d2bdad83ef9d4b1cba800507a33b6`
      );
